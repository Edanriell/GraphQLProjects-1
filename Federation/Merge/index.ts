...
const mergeSchemas = (examplePath: string) => {
	...
	const inputSchemas = fs
		.readdirSync(inputPath)
		.filter((f) => f.endsWith("graphql"))
		.map((f) => fs.readFileSync(path.join(inputPath, f), "utf-8"));
	let baseSchema = ``;
	for (const schema of inputSchemas) {
		const merge = mergeSDLs(baseSchema, schema);
		if (merge.__typename === "error") {
			throw new Error(
				`[${inputPath}], Cant merge the schemas. ${merge.errors.map((e) => `Conflict on: ${e.conflictingNode}.${e.conflictingField}`)}`
			);
		}
