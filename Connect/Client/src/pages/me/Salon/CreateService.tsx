...
const CreateServiceDialog = ({
	...
	createService: (service: ResolverInputTypes["CreateService"]) => Promise<string | undefined>;
}) => {
	const [dialogOpen, setDialogOpen] = useState(false);
	const [serviceForm, setServiceForm] = useState<Partial<ResolverInputTypes["CreateService"]>>();
	useEffect(() => {
		setServiceForm({});
	}, [dialogOpen]);
	return (
		<Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
			...
			<DialogContent className="sm:max-w-[425px]">
				...
				<div className="grid gap-4 py-4">
					...
					<div className="grid grid-cols-4 items-center gap-4">
						<Label htmlFor="approximateDurationInMinutes" className="text-right">
							Duration
						</Label>
						<Input
